...
### ✅ Telegram — проработан полностью

| Фича | Статус | Примечание |
|---|---|---|
| Сообщения из публичных каналов | ✅ | Telethon, `limit` per channel |
| Поиск по ключевым словам внутри канала | ✅ | `search_queries`, `search_limit` |
| Поиск каналов по теме | ✅ | `discover_channels` → `contacts.SearchRequest` |
| Replies / тредовый контекст | ✅ | `fetch_replies: true`, `reply_limit`; `reply_to_id` в metadata |
| Метаданные (просмотры, реакции, forwarded from) | ✅ | `fetch_metadata: true` → views, forwards, reactions (sum), forwarded_from |
| Media captions (подписи к фото/видео) | ✅ | `include_media_captions: true` → msg.message fallback |
| Фильтрация | ✅ | `min_views`, `min_length` |
| Rate limiting / flood protection | ✅ | `flood_sleep_threshold=60` — автосон при FloodWaitError |
